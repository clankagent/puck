...
function received(events){recorder.events(events);
 for(const e of events){
  $('result').textContent=`${names[e.direction]} · ${e.kind}`;
...
function apply(){
 ...
 for(const axis of Object.keys(directionAxes))$(axis+'Release').max=(options[axis+'Activation']-.01).toFixed(2);$('singleMode').value=options.singleMode;
