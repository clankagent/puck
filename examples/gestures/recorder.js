...
export function createRecorder({snapshot,begin,onSaved=()=>{}}){
 ...
 async function save(){
  ...
  try{
   ...
   status(`Saved ${result.id.slice(0,8)} · ${last.source} · ${reports} input reports. ${stopReason?stopReason+'. ':''}${pressMove?'Tell me “analyze my latest recording” in chat; no upload needed.':'Automatic analysis appears below; I can also read it directly.'}`);$('recordRetry').hidden=true;await history();onSaved(last,result);
 ...
 $('recordStart').onclick=()=>{
  ...
  const current=snapshot();captureSource=connected?'device':'simulator';started=performance.now();active=createGestureRecorder({startTimeMs:started,options:current.options,source:captureSource,note:((pressMove?'[press-move] ':'')+$('recordNote').value).slice(0,500)});reports=0;movingReports=0;lastReportAt=started;stopReason='';
