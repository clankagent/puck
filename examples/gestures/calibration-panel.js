import { calibrateGestures, createGestureTune, defaultGestureTune } from '../../dist/index.js';
...
export function createCalibrationPanel({applyTune}){
 ...
 function show(value,meta={}){
  try{recording=value;if(!$('combineRecordings').checked)captures=[];const id=meta.id??value.id??'unsaved';const existing=captures.findIndex(c=>c.id===id);if(existing>=0)captures[existing]={id,value};else captures.push({id,value});result=calibrateGestures(captures.map(c=>c.value));tune=result.tune;original=tune;
   ...
   $('calibrationIssues').textContent=result.issues.join(' ')||(result.missing.length?'Record the missing examples and enable “Combine with loaded recordings” to combine them.':'All eight action types have enough evidence. Counts describe inferred intent.');
   $('inspectAction').replaceChildren(new Option('Entire latest recording','-1'));result.actions.forEach((a,i)=>$('inspectAction').add(new Option(`${captures.length>1?`Recording ${a.recording+1} · `:""}${(a.start/1000).toFixed(2)}s · ${labels[a.direction]} ${a.kind}`,String(i))));
 ...
 $('applyTune').onclick=()=>{if(tune){applyTune(tune);$('calibrationIssues').textContent='Tune applied to live input. The original calibration is unchanged.';}};
