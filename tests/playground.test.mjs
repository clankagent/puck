...
for (const preset of Object.keys(gesturePresets)) test(`playground sequences exercise all 32 gestures with ${preset} tune`, () => {
  ...
  for (const gesture of gestures) {
    ...
    assert.deepEqual(events.filter(e => !['holdend','holdcancel'].includes(e.kind)).map(key), [key(gesture)]); counts.add(events);
  }
  assert.equal(Object.keys(counts.counts).length, 32); assert.ok(Object.values(counts.counts).every(n => n === 1));
