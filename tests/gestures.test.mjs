...
test('dominant axis wins; ambiguous diagonal waits',()=>{const g=createGestures({activation:.35,release:.12,neutralMs:35,doubleMs:320});g.update(n,0);g.update({...n,z:.5,rz:.5},10);assert.equal(g.state.phase,'neutral');g.update({...n,z:.7,rz:.2},20);assert.equal(g.state.direction,'push');});
