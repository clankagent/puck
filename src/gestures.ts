...
function createPulseGestures(configuration: GestureOptions | GestureTune): GestureRecognizer & { cancelExcursion(): void } {
  ...
  if(typeof standalone!=='boolean'||![standaloneMin,standaloneMax,standaloneNeutral,standaloneDouble].every(Number.isFinite)||standaloneMin<0||standaloneMax<standaloneMin||standaloneNeutral<0||standaloneDouble<0)throw new RangeError('Invalid standalone tilt options.');
  const isTilt=(d:GestureDirection)=>d.startsWith('rx')||d.startsWith('ry');
  ...
  if ([options.pressMode, options.pushMode, options.pullMode].some(v=>v!==undefined&&!['simple','tilt','auto'].includes(v))
    ...
    || tiltRelaxMs < 0 || tiltRelaxMs > tiltArmMs || (combinedEnabled && singleMode !== 'exclusive')) throw new RangeError('Invalid press-tilt options; combined gestures require exclusive single mode.');
  ...
  const deadline = (e: GestureEvent) => e.timestamp + (tiltMode(e.direction)?Math.max(doubleMs,tiltRelaxMs+tiltMinMs):doubleWindow(e.direction));
  ...
  return {
    ...
    update(input, t) {
      ...
      if (active) {
        ...
        let direction:GestureDirection = vertical ? (z > 0 ? 'push' : 'pull') : (rz > 0 ? 'clockwise' : 'counterclockwise');
        if(standalone){
          ...
          if(sideStrength>=(axis==='rx'?tx.activation:ty.activation)){
            if(sideStrength>strong){weak=Math.max(strong,Math.abs(axis==='rx'?ry:rx));strong=sideStrength;direction=(axis+((axis==='rx'?rx:ry)>0?'+':'-')) as TiltDirection;}
            else weak=Math.max(weak,sideStrength);
          }
        }
