...
export interface GestureGraphLane {direction:GestureDirection;label:string;center:number;low:number;high:number;activation:number;release:number;points:{t:number;value:number}[];actions:{start:number;end:number;kind:'single'|'double'}[]}
...
export function renderGestureGraphSvg(graph:GestureGraph,options:{width?:number;title?:string}={}):string {
  ...
  graph.lanes.forEach((lane,index)=>{
    ...
    for(const a of lane.actions){svg.push(`<rect x="${x(a.start)}" y="${top+24}" width="${Math.max(2,x(a.end)-x(a.start))}" height="76" fill="#cfdfef" opacity=".45"/>`);if(x(a.end)-x(a.start)>42)svg.push(`<text x="${x(a.start)+3}" y="${top+116}" font-size="11">${a.kind}</text>`);}
